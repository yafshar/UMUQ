...
{
    double s = 0;
    for (int i = 0; i < n; i++) s += pow(v[i]-mean,2);
...
int mvnrnd_cboet(gsl_rng * rng, const gsl_vector * mean, gsl_matrix * covar, gsl_vector * ANS)
{
    ...
    for(i=0; i<n; i++) {
        gsl_vector_set( &diagonal.vector, i, sqrt( gsl_vector_get(&diagonal.vector, i) ));
    }

    /* evec * matrix(diag(eval)) * transpose(evec)  */
    /*    gsl_blas_dsymm (CblasLeft, CblasUpper, 1.0, evec, eval_mx, 0.0, x_M);*/

    gsl_blas_dgemm (CblasNoTrans, CblasNoTrans, 1.0, evec, eval_mx, 0.0, x_M);
    gsl_blas_dgemm (CblasNoTrans, CblasTrans, 1.0, x_M, evec, 0.0, x_M_x);
    ...
    for(i=0;i<n;i++)
    {
        gsl_vector_set ( rnorms, i, gsl_ran_gaussian_ziggurat(rng, 1) );
    }
...
int countWords(char *str)
{
    ...
    while (*str)
    {
        ...
        // state is 0, then set the state as 1 and increment word count
        else if (state == 0)
        {
            state = 1;
            ++wc;
