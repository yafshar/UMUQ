...
void fitfun_initialize() {
    ...
    for (int p = 0; p < N_IND; p++) {
        ...
        for (int i = 0; i < NREC; i++)
            ffdata[p][i].x = (double*)malloc(PROBDIM*sizeof(double));
...
double loglike_psi(double *psi, int n, void *output);

double fitfun(double *x, int n, void *output, int *info) {

    double res;

    res = loglike_psi(x, n, output);
#if 0
    int BUFLEN_TMP=64;
...
inline double loglike_psi(double *psi, int n, void *output) {
    double out = 0;
    double * loglik;
    loglik = (double *) output;
    ...
    for (int p = 0; p < N_IND; p++) {
        ...
        loglik[p+2] = logEv[p] - log(NREC) + log(sum);
