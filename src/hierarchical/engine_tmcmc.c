...
void data_init()
{
    ...
    for (int i = 0; i < data.MaxStages+1; i++) {
...
void torc_update_curgen_db_task(double point[], double *pF, double *prior)
{
    double F = *pF;
...
void torc_update_curgen_db(double point[], double F, double *prior)
{
    ...
    int PRIORCN = data.prior_count;

    /* message to the database manager (separate process?) or direct execution by server thread */
    torc_create_direct(0, (void (*)())torc_update_curgen_db_task, 3,
                       data.Nth, MPI_DOUBLE, CALL_BY_COP,
                       1,        MPI_DOUBLE, CALL_BY_COP,
                       PRIORCN,  MPI_DOUBLE, CALL_BY_COP,
                       point, &F, prior);

    /* wait without releasing the worker */
    torc_waitall3();
}
...
void taskfun(double /*const*/ *x, int *pN, double *res, int winfo[4])
{
    ...
    if (PRIORCN > 1) {
        f = fitfun(x, N, res, winfo);
    } else {
        f = fitfun(x, N, (void *)NULL, winfo);
    }


/* peh: decide about this (results should be passed as argument to fitfun) */
...
void evaluate_F(double point[], double *Fval, int worker_id, int gen_id, int chain_id, int step_id, int ntasks)
{
    ...
    taskfun(point, &PROBDIM, Fval, winfo);
}
...
void initchaintask(double in_tparam[], int *pdim, double *out_tparam, int winfo[4])
{
    ...
    double point[data.Nth];
    double fpoint[PRIORCN+1];
    ...
    if (PRIORCN > 0) {
        fpoint[1] = logpriorpdf(point, data.Nth);

        torc_update_curgen_db(point, fpoint[0], fpoint+1);
    } else {
        torc_update_curgen_db(point, fpoint[0], fpoint);
    }

    if (data.ifdump) torc_update_full_db(point, fpoint[0], NULL, 0, 0);

    /* currently not required, the result is already in the db*/
    *out_tparam = fpoint[0];
...
void precompute_chain_covariances(const cgdbp_t* leader, double** init_mean, double** chain_cov, int newchains)
{
    ...
    int PROBDIM  = data.Nth;
    int NENTRIES = curgen_db.entries;

    double my_time = clock();

    // allocate space
    int*    nn_ind     = (int*)malloc(newchains*NENTRIES*sizeof(int));
    int*    nn_count   = (int*)malloc(newchains*sizeof(int));
    double* diam       = (double*)malloc(PROBDIM*sizeof(double));
    double* chain_mean = (double*)malloc(PROBDIM*sizeof(double));
    gsl_matrix* work   = gsl_matrix_alloc(PROBDIM, PROBDIM);
    ...
    for (scale = 0.1; scale <= 1.0; scale += ds) {
        // find neighbors in a rectangle - O(NENTRIES^2)
        for (pos = 0; pos < newchains; ++pos) {
            nn_count[pos] = 0;
            double* curr = leader[pos].point;
            for (i = 0; i < NENTRIES; ++i) {
                double* s = curgen_db.entry[i].point;
                if (in_rect(curr, s, diam, scale, PROBDIM)) {
                    nn_ind[pos*NENTRIES+nn_count[pos]] = i;
                    nn_count[pos]++;
                }
            }
        }
        ...
        for (pos = 0; pos < newchains; ++pos) {
            ...
            for (i = 0; i < PROBDIM; i++)
                for (j = 0; j < PROBDIM; j++) {
                    ...
                    for (k = 0; k < nn_count[pos]; k++) {
                        ind = nn_ind[pos*NENTRIES+k];
                        s += (curgen_db.entry[ind].point[i]-chain_mean[i])*(curgen_db.entry[ind].point[j]-chain_mean[j]);
                    }
                    chain_cov[pos][i*PROBDIM+j] = chain_cov[pos][j*PROBDIM+i] = s/nn_count[pos];
                }
    ...
    if (status != GSL_SUCCESS) {
        ...
        for (i = 0; i < PROBDIM; i++)
            for (j = 0; j < PROBDIM; j++)
                chain_cov[pos][k++] = data.bbeta*runinfo.SS[i][j];
    }
    ...
    for (pos=0; pos<5; ++pos)
    {
        ...
        print_matrix((char *)"chain_covariance", chain_cov[pos], PROBDIM*PROBDIM);
    }
#endif
...
int compute_candidate(double candidate[], double chain_mean[], double var)
{
    int i, j, k;
    int PROBDIM = data.Nth;
    double bSS[PROBDIM*PROBDIM];

    k=0;
    for (i = 0; i < PROBDIM; i++)
        for (j = 0; j < PROBDIM; j++)
            bSS[k++] = data.bbeta*runinfo.SS[i][j];

    //retry:
    mvnrnd(chain_mean, (double *)bSS, candidate, PROBDIM);
    ...
    //    if (i < PROBDIM) goto retry;
    if (i < PROBDIM) return -1;
...
int compute_candidate_cov(double candidate[], double chain_mean[], double chain_cov[])
{
    ...
    mvnrnd(chain_mean, (double *)chain_cov, candidate, PROBDIM);
...
void chaintask(double in_tparam[], int *pdim, int *pnsteps, double *out_tparam, int winfo[4], double *init_mean, double *chain_cov)
{
    ...
    /* old*/
    double leader[PROBDIM];
    double loglik_leader[PRIORCN+1];
    /* new*/
    double candidate[PROBDIM];
    double loglik_candidate[PRIORCN+1];

    for (i = 0; i < PROBDIM; i++)
        leader[i] = in_tparam[i];    /*chainwork->in_tparam[i];*/    /* get initial leader */

    loglik_leader[0] = *out_tparam;      /*chainwork->out_tparam[0];*/    /* and its value */

    double pj = runinfo.p[runinfo.Gen];

#define BURN_IN 4
    for (step = 0; step < nsteps + BURN_IN; step++) {
        double chain_mean[PROBDIM];
        if (step == 0)
            for (i = 0; i < PROBDIM; ++i)
                chain_mean[i] = init_mean[i];
        else
            for (i = 0; i < PROBDIM; ++i)
                chain_mean[i] = leader[i];

#if 0
        int fail = compute_candidate_cov(candidate, chain_mean, chain_cov);
#else
        // I keep this for the moment, for performance reasons
        int fail = compute_candidate(candidate, chain_mean, 1);
#endif

        if (!fail)
        {
            /* this can spawn many tasks*/
            evaluate_F(candidate, loglik_candidate, me, gen_id, chain_id, step, 1);

            /* last argument should be 1 if it is a surrogate */
            if (data.ifdump && step >= BURN_IN)
                torc_update_full_db(candidate, loglik_candidate[0], NULL, 0, 0);

            double L;

            if (PRIORCN > 0) {
                /* Decide */
                loglik_candidate[1] = logpriorpdf(candidate, PROBDIM);
                loglik_leader[1]    = logpriorpdf(leader,    PROBDIM);

                /* with exp, with log in logpriorpdf and fitfun */
                L = exp((loglik_candidate[1]-loglik_leader[1])+(loglik_candidate[0]-loglik_leader[0])*pj);
            } else {
                /* Decide */
                double logprior_candidate = logpriorpdf(candidate, PROBDIM);    /* from PanosA */
                double logprior_leader    = logpriorpdf(leader,    PROBDIM);
                /* with exp, with log in logpriorpdf and fitfun */
                L = exp((logprior_candidate-logprior_leader)+(loglik_candidate[0]-loglik_leader[0])*pj);
            }
            ...
            if (P < L) {
                /* new leader! */
                for (i = 0; i < PROBDIM; i++)
                    leader[i] = candidate[i];

                for (i=0; i< PRIORCN+1; i++)
                    loglik_leader[i] = loglik_candidate[i];

                if (step >= BURN_IN) {
                    if (PRIORCN > 0)
                        torc_update_curgen_db(leader, loglik_leader[0], loglik_leader+1);
                    else
                        torc_update_curgen_db(leader, loglik_leader[0], loglik_leader);
                }
            } else {
...
int prepare_newgen(int nchains, cgdbp_t *leaders)
{
    ...
    double       *fj  = (double *)       malloc(NENTRIES*sizeof(double));
    unsigned int *sel = (unsigned int *) malloc(NENTRIES*sizeof(sel));

    double **g_x;
    g_x = (double **)malloc(PROBDIM*sizeof(double *));
    for (i = 0; i < PROBDIM; i++)
        g_x[i] = (double *)malloc(NENTRIES*sizeof(double));
    ...
    {/*start block*/
        ...
        double meanx[PROBDIM], stdx[PROBDIM];
        for (p = 0; p < PROBDIM; p++) {
            meanx[p] = compute_mean(x[p], NENTRIES);
            stdx[p]  = compute_std(x[p],  NENTRIES, meanx[p]);
        }

        printf("CURGEN DB (COMPLE) %d\n", runinfo.Gen);
        print_matrix((char *)"means", meanx, PROBDIM);
        print_matrix((char *)"std", stdx, PROBDIM);
    }/*end block*/
    ...
    {/*start block*/
        ...
        printf("CURGEN DB (UNIQUE) %d: [un size = %d]\n", runinfo.Gen, un); /* + 1);*/
        print_matrix((char *)"means", meanx, PROBDIM);
        print_matrix((char *)"std",   stdx,  PROBDIM);
    } /* end block*/
    ...
    calculate_statistics(fj, NENTRIES, data.Num[runinfo.Gen], runinfo.Gen, sel);
    ...
    qsort(list, NENTRIES, sizeof(sort_t), compar_desc);
    ...
    ldi = 0;
    for (i = 0; i < NENTRIES; i++) {    /* newleader */
        if (list[i].nsel != 0) {
            int idx = list[i].idx;
            for (p = 0; p < PROBDIM; p++) {
    ...
    for (i = 0; i < PROBDIM; i++) free(g_x[i]);
    free(g_x);
...
int main(int argc, char *argv[])
{
    ...
    if (goto_next == 0)
    {
        ...
        // double mmin=-FAIL;
        // double mmax=FAIL;
        // tmpy = (double *)malloc(curgen_db.entries*sizeof(double));
        // for (int yy=0; yy<curgen_db.entries; yy++) {
        //     tmpy[yy] = curgen_db.entry[yy].F;
        // }
        // for (int yy=0; yy<curgen_db.entries; yy++) {
        //     if (tmpy[yy] > FAIL) {
        //         if (tmpy[yy] > mmax) 
        ...
        //     }
        // }
        // double mminmax = (mmax-mmin);
        // for (int yy=0; yy<curgen_db.entries; yy++) {
        //     if (tmpy[yy] > FAIL) {
        //         tmpy[yy] = (tmpy[yy] - mmin)/mminmax;
        //     }
        // }
        // for (int yy=0; yy < curgen_db.entries; yy++) {
        //     curgen_db.entry[yy].F = tmpy[yy];
        // }
        // free(tmpy);
        // tmpy = (double *)malloc(full_db.entries*sizeof(double));
        // for (int yy=0; yy<full_db.entries; yy++) {
        //     tmpy[yy] = full_db.entry[yy].F;
        // }
        // mmin=-FAIL;
        // mmax=FAIL;
        // for (int yy=0; yy<full_db.entries; yy++) {
    ...
    for (; runinfo.Gen < data.MaxStages; runinfo.Gen++){
        ...
        for (i = 0; i < nchains; i++) {
            ...
            torc_create(leaders[i].queue, (void (*)())chaintask, 7,
                        ...
                        in_tparam, &data.Nth, &nsteps, &out_tparam[i],
                        winfo, init_mean, chain_cov);
        }
