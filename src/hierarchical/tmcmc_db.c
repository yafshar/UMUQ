...
void update_full_db(double point[], double F, double *G, int n, int surrogate)
{
    ...
    if (full_db.entry[pos].point == NULL)
        full_db.entry[pos].point = (double *) malloc(data.Nth*sizeof(double));

    for (i = 0; i < PROBDIM; i++)
        full_db.entry[pos].point[i] = point[i];

    full_db.entry[pos].F = F;
    full_db.entry[pos].nG = n;
    for (i = 0; i < n; i++)
        full_db.entry[pos].G[i] = G[i];
...
void update_curgen_db(double point[], double F, double *prior)
{
    ...
    if (curgen_db.entry[pos].point == NULL)
        curgen_db.entry[pos].point = (double *) malloc(PROBDIM*sizeof(double));

    for (i = 0; i < PROBDIM; i++)
        curgen_db.entry[pos].point[i] = point[i];

    curgen_db.entry[pos].F = F;

    if (PRIORCN > 0)
        if (curgen_db.entry[pos].prior == NULL)
            curgen_db.entry[pos].prior = (double *) malloc(PRIORCN*sizeof(double));

    for (i = 0; i < PRIORCN; i++)
        curgen_db.entry[pos].prior[i] = prior[i];
}
...
void update_curres_db(double point[EXPERIMENTAL_RESULTS], double F)
{
    ...
    if (curres_db.entry[pos].point == NULL)
        curres_db.entry[pos].point = (double *) malloc((EXPERIMENTAL_RESULTS+1)*sizeof(double));
...
void print_full_db()
{
    ...
    if (data.Nth == 2) {
        for (int pos = 0; pos < full_db.entries; pos++) {
            printf("ENTRY %d: POINT(%20.16lf,%20.16lf) F=%20.16lf SG=%d\n", pos, full_db.entry[pos].point[0], full_db.entry[pos].point[1], full_db.entry[pos].F, full_db.entry[pos].surrogate);
            printf("\tG=[");
...
void dump_curgen_db(int Gen)
{
    ...
    if (PRIORCN > 0) {
        for (int pos = 0; pos < curgen_db.entries; pos++) {
            ...
            for (int i = 0; i < PRIORCN; i++)
                fprintf(fp, "%20.16lf ", curgen_db.entry[pos].prior[i]);
            fprintf(fp,"\n");
        }
    } else {
        for (int pos = 0; pos < curgen_db.entries; pos++) {
...
int load_curgen_db(int Gen)
{
...
#if DBG
        if (countWords(line) - PROBDIM - 1 /= PRIORCN)
            printf("DB file: %s not found!!!\n", fname);
            exit(1);
            return 1;
        }
    ...
    if (PRIORCN > 0) {
        for (int pos = 0; pos < curgen_db.entries; pos++) {
            ...
            for (int i = 0; i < curgen_db.entry[pos].counter; i++)
                fscanf(fp, "%lf", &curgen_db.entry[pos].prior[i]);
        }
    } else {
        for (int pos = 0; pos < curgen_db.entries; pos++) {
            if (curgen_db.entry[pos].point == NULL)
