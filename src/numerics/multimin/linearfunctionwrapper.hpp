...
class linearFunctionWrapper
{
  public:
     ...
     */
    explicit linearFunctionWrapper(umuqDifferentiableFunction<T, F_MTYPE<T>, DF_MTYPE<T>, FDF_MTYPE<T>> &umFun);
...
template <typename T>
linearFunctionWrapper<T>::linearFunctionWrapper(umuqDifferentiableFunction<T, F_MTYPE<T>, DF_MTYPE<T>, FDF_MTYPE<T>> &umFun) : fun(umFun) {}
...
bool linearFunctionWrapper<T>::fdf(T const alpha, T *F, T *DF)
{
    ...
    fun.fdf(x_alpha, &f_alpha, g_alpha);
