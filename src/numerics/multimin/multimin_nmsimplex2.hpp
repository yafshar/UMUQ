...
class nmsimplex2rand : public multimin_fminimizer_type<T, nmsimplex2rand<T, TMF>, TMF>
{
  public:
     ...
     */
    nmsimplex2rand(const char *name_ = "nmsimplex2rand") : name(name_),
                                                           x1(nullptr),
                                                           y1(nullptr),
                                                           ws1(nullptr),
                                                           ws2(nullptr),
    ...
    T try_corner_move(T const coeff, std::size_t corner, T *xc, multimin_function<T, TMF> const *f)
    {
    ...
    T compute_size()
    {
        ...
        for (std::size_t i = 0; i < N; i++)
        {
            ...
            T t(0);
            std::for_each(s, s + n, [&](T const s_i) { t += s_i * s_i; });
