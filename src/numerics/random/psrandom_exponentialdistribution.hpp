...
class exponentialDistribution
{
   public:
      ...
      * double, or long double
      * 
	 * \param idata  Array of data of random numbers according to probability density function \f$ \frac{1}{\mu}e^{\left(-\frac{x}{\mu}\right)} \f$ 
...
template <typename RealType>
exponentialDistribution<RealType>::~exponentialDistribution() {}
...
template <typename RealType>
inline void exponentialDistribution<RealType>::dist(RealType *idata, int const nSize)
{
     // Get the thread ID
     int const me = PRNG_initialized ? torc_i_worker_id() : 0;
...
template <typename RealType>
inline void exponentialDistribution<RealType>::dist(std::vector<RealType> &idata)
{
     // Get the thread ID
     int const me = PRNG_initialized ? torc_i_worker_id() : 0;
