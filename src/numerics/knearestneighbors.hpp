 ...
 * away the point is from the mean of the distribution and is essentially a stretched Euclidean distance.<br>  
 * Using a covariance matrix \f$ S \f$ of size \f$ d \times d \f$, where 
 * \f$ d \f$ is the dimensionality of the space it will be evaluating, and given 
 * two vectors \f$ x \f$ and \f$ y \f$ with the same dimensionality of \f$ d, \f$<br>
 * \f$ d(x, y) = \sqrt{(x - y)^T S^{-1} (x - y)},\f$ where \f$ S \f$ is the covariance matrix.
 *
 * In this implamentation, we use the L2 distance and simply stretch the actual dataset
 * itself before performing any evaluations.
 * 
 * \tparam T Data type
 */
template <typename T>
class MahalanobisNearestNeighbor : public kNearestNeighbor<T, flann::L2<T>>
{
  ...
  private:
    /*!
     * \brief The covariance matrix associated with Mahalanobis distance.
     * 
     * To avoid calculating the full rating matrix, for Mahalanobis distance metric 
     * int K nearest neighbor search, we will do nearest neighbor search only on the 
     * H matrix, using the observation that if the matrix X = W*H, 
     * then d(X.col(i), X.col(j)) = d(W H.col(i), W H.col(j)).  
     * This can be seen as nearest neighbor search on the H matrix with the Mahalanobis 
     * distance where M^{-1} = W^T W.  So, we'll decompose M^{-1} = L L^T (the Cholesky 
     * decomposition), and then multiply H by L^T. Then we can perform nearest neighbor search.
     * 
     */
    EMatrixX<T> covariance;
...
void MahalanobisNearestNeighbor<T>::setCovariance(EMatrixX<T> const &Covariance)
{
    ...
    // Compute the Cholesky decomposition \f$ (LL^T) \f$ of inverse covariance matrix and
...
// T operator()(Iterator1 a, Iterator2 b, std::size_t size, T worst_dist = -1) const
// {
//     T result = T{};
