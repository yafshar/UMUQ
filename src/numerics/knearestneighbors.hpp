...
class kNearestNeighbor
{
  public:
     ...
     * \param nN            Number of nearest neighbors to find
     *  
     * \return true         if replacement goes with no problem
     * \return false        in case of failing to allocate memory
     */
    bool reset(int const ndataPoints, int const nDim, int const nN)
    {
        drows = ndataPoints;
        qrows = ndataPoints;
        cols = nDim;
        ...
        try
        {
            indices_ptr.reset(new int[ndataPoints * (nN + 1)]);
            dists_ptr.reset(new T[ndataPoints * (nN + 1)]);
        }
        catch (std::bad_alloc &e)
        {
            std::cerr << "Error : " << __FILE__ << ":" << __LINE__ << " : " << std::endl;
...
#ifdef HAVE_FLANN
        indices.rows = ndataPoints;
        indices.cols = nN + 1;
        indices.data = static_cast<unsigned char *>(indices_ptr.get());

        dists.rows = ndataPoints;
        dists.cols = nN + 1;
        dists.data = static_cast<unsigned char *>(dists_ptr.get());
#endif
        the_same = true;
     ...
     * \return false        in case of failing to allocate memory
     */
    bool reset(int const ndataPoints, int const nqueryPoints, int const nDim, int const nN)
    {
        drows = ndataPoints;
        qrows = nqueryPoints;
        cols = nDim;
        ...
        try
        {
            indices_ptr.reset(new int[nqueryPoints * nN]);
            dists_ptr.reset(new T[nqueryPoints * nN]);
        }
        catch (std::bad_alloc &e)
        {
            std::cerr << "Error : " << __FILE__ << ":" << __LINE__ << " : " << std::endl;
...
#ifdef HAVE_FLANN
        indices.rows = nqueryPoints;
        indices.cols = nN;
        indices.data = static_cast<unsigned char *>(indices_ptr.get());

        dists.rows = nqueryPoints;
        dists.cols = nN;
        dists.data = static_cast<unsigned char *>(dists_ptr.get());
#endif
        the_same = false;
