  ...
  public:
    ...
    bool dcops(T *idata, int const nPoints, T *kernel, int *beta, int order = 2, int nENN = 2, T ratio = static_cast<T>(1))
    {
        ...
        B0.resize(msize);
        {
            ...
            //{It is mentioned that it should be always zero in order to obtain
            //kernels with a vanishing zeroth-order moment that can be consistently
            //evaluated at off-particle locations!}
            //
        ...
        for (int i = 0; i < nPoints; i++)
        {
            ...
            for (int j = 0; j < nNN; j++)
            {
                ...
                VMT.block(0, j, msize, 1) << Eigen::Map<EMatrixX<T>>(col);
            }
